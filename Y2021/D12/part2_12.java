...
public class part2_12 {
	public static void main(String[] args) {
		...
		try {
			...
			scanner.close();
			System.out.println(dfs("start", new HashSet<>(Collections.singleton("start")), graph, false));
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
	}

	public static int dfs(String vertex, Set<String> visits, Dictionary<String, List<String>> graph, boolean extra) {
		...
		for(String edge : graph.get(vertex)) {
			if(edge.toLowerCase().equals(edge) && !edge.equals("end")) {
				if(!visits.contains(edge)) {
					...
					newVisits.add(edge);
					count += dfs(edge, newVisits, graph, extra);
				} else if (!extra && !edge.equals("start")) {
					Set<String> newVisits = new HashSet<>(visits);
					newVisits.add(edge);
					count += dfs(edge, newVisits, graph, true);
				}
			} else {
				count += dfs(edge, visits, graph, extra);
			}
		}
		return count;
	}
