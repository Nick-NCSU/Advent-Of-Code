...
input.sort((a, b) => {
  ...
  const jokerA = handA.J;
  if(jokerA && jokerA !== 5) {
    delete handA.J;
    handA[Object.keys(handA).reduce((a, b) => handA[a] > handA[b] ? a : b)] += jokerA;
  }
  const jokerB = handB.J;
  if(jokerB && jokerB !== 5) {
    delete handB.J;
    handB[Object.keys(handB).reduce((a, b) => handB[a] > handB[b] ? a : b)] += jokerB;
  }
